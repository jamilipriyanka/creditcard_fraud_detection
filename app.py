...
with tab1:
    ...
    if submitted:
        ...
        with col1:
            # Create a gauge chart for the fraud probability
            if fraud_probability is not None:
                # Create simple horizontal tabs for result visualization
                result_viz_tabs = st.tabs(["Risk Gauge", "Model Accuracy", "Prediction Confidence"])
                
                # Tab 1: Risk Gauge
                with result_viz_tabs[0]:
                    st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)
                    
                    # Create a gauge chart for fraud risk
                    fig = go.Figure(go.Indicator(
                        mode="gauge+number",
                        value=fraud_probability * 100,
                        domain={'x': [0, 1], 'y': [0, 1]},
                ...
                with result_viz_tabs[1]:
                    st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)
                    
                    # Create model performance metrics
                    st.markdown("<h4 style='color: #694D43; margin-bottom: 20px;'>Model Performance Metrics</h4>", unsafe_allow_html=True)
                    
                    # Simplified layout with chart on left, metrics on right
                    viz_cols = st.columns([3, 2])
                    
                    with viz_cols[0]:
                        # Create a clean radar chart for model metrics
                        # Define sample accuracy data
                        accuracy = 0.947
                        precision = 0.923
                        recall = 0.916
                        f1_score = 0.919
                        ...
                        fig.add_trace(go.Scatterpolar(
                            r=[accuracy*100, precision*100, recall*100, f1_score*100, accuracy*100],
                            theta=['Accuracy', 'Precision', 'Recall', 'F1 Score', 'Accuracy'],
                            fill='toself',
                            line_color='#FF7B7A',
                            fillcolor='rgba(255, 123, 122, 0.3)'
                        ))
                    ...
                    with viz_cols[1]:
                        ...
                        st.metric("Model Accuracy", f"94.7%")
                        st.markdown("<div style='margin-top: 15px;'></div>", unsafe_allow_html=True)
                        st.metric("Recall Score", f"91.6%")
                
                # Tab 3: Prediction Confidence
                with result_viz_tabs[2]:
                    st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)
                    
                    # Display top influential features more simply
                    st.markdown("<h4 style='color: #694D43; margin-bottom: 20px;'>Key Factors In Prediction</h4>", unsafe_allow_html=True)
                    
                    # Create horizontal bar chart for top features
                    feature_impact = {
                        'Feature': ['Transaction Amount', 'Time of Day', 'Merchant Category', 'IP Match'],
                        'Impact': [0.28, 0.17, 0.12, 0.11]
                    }
                    ...
                    fig = px.bar(
                        ...
                        x='Impact',
                        orientation='h',
                        color_discrete_sequence=['#FF7B7A'],
                        text_auto='.0%'
                    )
                    ...
                    st.metric("Prediction Confidence", "87%", 
                             help="Higher confidence means the model is more certain about the prediction")
